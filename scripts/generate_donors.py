...
def generate_donor(i: int, golden: bool = False) -> dict:
...
def count_demo_matches(donors: list[dict]) -> int:
    return sum(
        1
        for d in donors
        if d["location"]["state"] == "TX"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="High-affinity TX records seeded for demo queries",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/donors.ndjson"),
        help="Output NDJSON path for bulk indexing",
    )
    parser.add_argument(
        "--index",
        default="athletic-boosters",
    ...
    donors = [
        generate_donor(i, golden=(i in golden_ids)) for i in range(args.count)
    ]

    write_ndjson(donors, args.output, args.index)

    tiers = summarize_tiers(donors)
    demo_matches = count_demo_matches(donors)
    golden_scores = sorted(
        (d["affinity_score"] for i, d in enumerate(donors) if i in golden_ids),
        reverse=True,
    )[:5]

    print(f"Generated {len(donors)} donors → {args.output}")
    print(f"Affinity tiers: low={tiers['low']}, medium={tiers['medium']}, high={tiers['high']}")
    print(f"Demo query matches (TX, 3+ games, $2M+ RE): {demo_matches}")
    if golden_scores:
